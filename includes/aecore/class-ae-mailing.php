...
class AE_Mailing extends AE_Base
{
    ...
    public function wp_mail($to, $subject, $content, $filter = array() , $headers = '') {
        ...
        if (isset($filter['user_id'])) {
            $content = $this->filter_authentication_placeholder($content, $filter['user_id'], $filter['user_pass']);
        }
     ...
     * @since 1.0
     */
    function filter_authentication_placeholder($content, $user_id, $user_pass = '' ) {
        $user = new WP_User($user_id);
        ...
        $content = str_ireplace('[user_pass]', $user_pass, $content);
